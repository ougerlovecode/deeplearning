...
# https://blog.csdn.net/gaishi_hero/article/details/82055607
class HandWritingNumberRecognize_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "test":
            img = Image.open(self.root_path + 'test_' + str(index) + '.bmp')
            img = transform(img)
            return img
        elif self.mode == 'train':
            img = Image.open(self.root_path + 'train_' + str(index) + '.bmp')
            img = transform(img)
            return img, self.label[index]
        else:
            img = Image.open(self.root_path + 'val_' + str(index) + '.bmp')
            img = transform(img)
            return img, self.label[index]
...
def validation(model,loader):
    ...
    print("Test Loss:{:.6f},Acc:{:.6f}".format(eval_loss/len(loader.dataset),eval_acc/len(loader.dataset)))
...
def alltest(model):
    ...
    for index in range(len(dataset_test)):
        ...
        with open('test_result.txt', 'a') as f:
            ...
            f.write('\n')
        if index % 200 == 0:
            print('[{} / {} ({:.0f}%)]'.format(index,len(dataset_test),100. * index / len(dataset_test)))
...
def train(epoch_num):
    ...
    if epoch_num %10 ==0:
        torch.save(model.state_dict(),"model2/epoch"+ str(epoch_num) + "model_params.pkl")
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("model2/epoch40model_params.pkl"))
    # validation(model, data_loader_val)
