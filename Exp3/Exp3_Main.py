...
if __name__ == "__main__":
    ...
    # 训练集验证集
    train_dataset = TextDataSet(filepath=config.path_root+"./data/new_data_train.txt")
    train_loader = DataLoader(dataset=train_dataset, batch_size=config.batch_size)
    # print(len(train_loader))
    val_dataset = TextDataSet(filepath=config.path_root+"./data/new_data_val.txt")
    val_loader = DataLoader(dataset=val_dataset, batch_size=config.batch_size)

    # 测试集数据集和加载器
    test_dataset = TestDataSet(config.path_root+"./data/test_exp3.txt")
    ...
    for i in range(start_epoch, start_epoch + config.epoch):
        train(Text_Model, loader=train_loader,epoch_no=i,epoch_num=config.epoch)
